...
public class AstJavaSlicer extends Slicer {
 ...
 /*
  ...
  public static Set gatherAssertions(CallGraph CG, Collection partialRoots) {
    ...
    for(Iterator ns = DFS.getReachableNodes(CG, partialRoots).iterator();
	ns.hasNext(); )
    {
  ...
  public static Pair<Collection<Statement>, SDG> 
    computeAssertionSlice(CallGraph CG, PointerAnalysis pa, Collection partialRoots)
  {
    ...
    Trace.println("SDG:\n" + sdg);
    return Pair.make(AstJavaSlicer.computeBackwardSlice(sdg, gatherAssertions(CG, partialRoots), pcg, pa, DataDependenceOptions.FULL, ControlDependenceOptions.FULL), sdg);
  }
