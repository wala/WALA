...
class ReflectiveInvocationSelector implements ContextSelector {
   ...
   * <ol>
   * <li> If the method being invoked through reflection is definitely static, then
   *      do not create a callee target for any <code>callee</code> method that is not static.
   *      In this case, return <code>null</code>.
   * <li> If the method being invoked through reflection takes a constant number of
   *      parameters, <code>n</code>, then do not create a callee target for any
   *      <code>callee</code> method that takes a number of parameters different from
  ...
  public boolean mayUnderstand(CGNode caller, CallSiteReference site, IMethod targetMethod, InstanceKey instance) {
    if (targetMethod.getReference().equals(ReflectiveInvocationInterpreter.METHOD_INVOKE) ||
        isConstructorConstant(instance) && targetMethod.getReference().equals(ReflectiveInvocationInterpreter.CTOR_NEW_INSTANCE)) {
        return true;
    }
    return false;
  }
