...
public class ReflectionTest extends WalaTestCase {
   ...
   * method from GFG class. This is to test the support for
   * Class.getAnnotation and Method.getAnnotation.
   */
  //@Test
  public void testReflect26()
    ...
    CallGraph cg =
        CallGraphTestUtil.buildZeroOneCFA(options, new AnalysisCacheImpl(), cha, scope, false);
    TypeReference tr =
        TypeReference.findOrCreate(ClassLoaderReference.Application, "Lreflection/GFG");
    MethodReference mr =
        MethodReference.findOrCreate(tr, "getCustomAnnotation", "()V");
    Set<CGNode> nodes = cg.getNodes(mr);
    Assert.assertFalse(nodes.isEmpty());
  }

  /**
   * Test that when analyzing Reflect27, the call graph includes a node for the values method
   * from ENUM class Color. This is to test the support for
  ...
  public void testReflect27()
    ...
    CallGraph cg =
        CallGraphTestUtil.buildZeroOneCFA(options, new AnalysisCacheImpl(), cha, scope, false);
    TypeReference tr =
        TypeReference.findOrCreate(ClassLoaderReference.Application, "Lreflection/Color");
    MethodReference mr =
        MethodReference.findOrCreate(tr, "values", "([Lreflection.Reflect27$Color;)V");
    Set<CGNode> nodes = cg.getNodes(mr);
    Assert.assertFalse(nodes.isEmpty());
  }
