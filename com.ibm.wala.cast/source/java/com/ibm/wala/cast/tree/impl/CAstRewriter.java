...
public abstract class CAstRewriter<RewriteContext> {
  ...
  private CAstControlFlowMap
    copyFlow(Map<CAstNode, CAstNode> nodeMap, CAstControlFlowMap orig)
  {
  ...
  private Map copyChildren(Map nodeMap, Map children) {
	...
	for(Iterator oldEntities = 
	      ((Collection)children.get(oldNode)).iterator();
	    oldEntities.hasNext(); )
	{
			 ...
			 final Map children) 
  {
	...
	if (theTypes == null) theTypes = copyTypes(nodes, types);
	return theTypes;
      }

      public Map newChildren() {
	if (theChildren == null) theChildren = copyChildren(nodes, children);
	return theChildren;
      }
    };
  }
  ...
  public CAstEntity rewrite(final CAstEntity root) {
	...
	public CAstControlFlowMap getControlFlow() {
      ...
      Map children = root.getAllScopedEntities();
      final Map newChildren = new LinkedHashMap();
      for(Iterator keys = children.entrySet().iterator(); keys.hasNext();) {
	Map.Entry entry = (Map.Entry)keys.next();
	Object key = entry.getKey();
	...
	for(Iterator es = ((Collection)entry.getValue()).iterator();
	    es.hasNext();)
	{
